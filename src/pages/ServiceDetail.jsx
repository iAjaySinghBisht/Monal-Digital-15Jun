...
import { Eyebrow, Pill, ArrowGlyph } from "../components/Decor";
...
const DisciplineRow = ({ index, item, isOpen, onToggle }) => (
  <div
    data-reveal="up"
...
const ServiceDetail = () => {
  ...
  const key = ORDER.find((k) => services[k].slug === slug);
  ...
  const prev = ORDER[(orderIndex - 1 + ORDER.length) % ORDER.length];
    ...
    <>
      ...
      <section className="relative bg-ink text-paper">
        <div className="grid sm:grid-cols-2">
          <Link
            to={`/services/${services[prev].slug}`}
          ...
          >
            ...
            <span className="relative flex items-baseline gap-4">
              <span className="font-display text-3xl md:text-[2.4rem] leading-none tracking-tight group-hover:text-sun transition-colors">
                {services[next].eyebrow}
              </span>
              <span className="font-display text-xl md:text-2xl tabular-nums text-white/25">
                {services[next].number}
              </span>
            </span>
          </Link>
        </div>
